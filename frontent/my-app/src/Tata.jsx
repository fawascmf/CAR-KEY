...
function Tata() {
  const [tatproduct, setTataprodect] = useState([]);
  const {_id } = useParams();
  const navigate = useNavigate();
  useEffect(() => {
    axios
      .get(`http://localhost:5000/tataview`)
      .then((val) => setTataprodect(val.data))
        ...
        <MDBCol md="3">
          <MDBCard className="text-black">
            ...
            <MDBCardBody>
              ...
              <br />
              <div>
        <Link to={`/tatadetilpage/${viewp._id}`}><center><button  type="button" class="btn btn-light"><span style={{color:'#128650',fontWeight:'bold'}}>VIEW</span></button></center></Link>
              </div>
            </MDBCardBody>
          </MDBCard>
        </MDBCol>
